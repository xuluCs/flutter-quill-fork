...
class _TextLineState extends State<TextLine> {
  ...
  TextSpan _buildTextSpan(DefaultStyles defaultStyles, LinkedList<Node> nodes, TextStyle lineStyle) {
    ...
    final children = nodes.map((node) => _getTextSpanFromNode(defaultStyles, node, widget.line.style)).toList(growable: false);
  ...
  TextStyle _getInlineTextStyle(leaf.Text textNode, DefaultStyles defaultStyles, Style nodeStyle, Style lineStyle, bool isLink) {
...
class RenderEditableTextLine extends RenderEditableBox {
  ...
  RenderEditableTextLine(this.line, this.textDirection, this.textSelection, this.enableInteractiveSelection, this.hasFocus, this.devicePixelRatio, this.padding,
      this.color, this.cursorCont, this.inlineCodeStyle);
  ...
  TextSelectionPoint _getEndpointForSelection(TextSelection textSelection, bool first) {
    if (textSelection.isCollapsed) {
      return TextSelectionPoint(Offset(0, preferredLineHeight(textSelection.extent)) + getOffsetForCaret(textSelection.extent), null);
    }
  ...
  TextRange getLineBoundary(TextPosition position) {
    ...
    final lineBoxes = _getBoxes(TextSelection(baseOffset: 0, extentOffset: line.length - 1))
        ...
        .toList(growable: false);
    return TextRange(
        start: getPositionForOffset(Offset(lineBoxes.first.left, lineDy)).offset, end: getPositionForOffset(Offset(lineBoxes.last.right, lineDy)).offset);
  }
        ...
        for (final item in line.children) {
          ...
          for (final box in rects) {
            ...
            if (inlineCodeStyle.radius == null) {
              ...
              final paintRect = RRect.fromLTRBR(rect.left - 2, rect.top, rect.right + 2, rect.bottom, inlineCodeStyle.radius!);
  ...
  void _paintCursor(PaintingContext context, Offset effectiveOffset, bool lineHasEmbed) {
    final position = cursorCont.isFloatingCursorActive
        ? TextPosition(
            offset: cursorCont.floatingCursorTextPosition.value!.offset - line.documentOffset, affinity: cursorCont.floatingCursorTextPosition.value!.affinity)
